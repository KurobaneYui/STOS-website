...
class self_study_data_single { // 早自习数据类（单日）
    ...
    function __construct($riqi,$jiaoxuelou,$qvhao,$jiaoshibianhao,$personID) { // 构造函数
        ...
        $connect = new STOS_MySQL();
        $re = $connect->search("早自习教室安排",array("学院","早自习人数"),array("教学楼"=>$this->jiaoxuelou,"区号"=>$this->qvhao,"教室编号"=>$this->jiaoshibianhao,"起始日期"=>getWeekRange(strtotime($this->riqi),1)[0]),false)->fetch_assoc();
        $this->xueyuan = $re["学院"];
        $this->yingdaorenshu = $re["早自习人数"];
        unset($re);
        $connect->__destruct();
    }
...
class self_study_data_set { // 早自习数据类（单人一周内可录入数据集合）
    ...
    function __construct($personID, $current_time) {
        ...
        $result = $connection_data->search("早自习排班",false,array("查早组员"=>$personID,"周起始日期"=>$this->data_Monday),false)->fetch_assoc();
        $this->jiaoxuelou = $result["教学楼"];
        $this->qvhao = $result["区号"];
        $this->jiaoshibianhao = $result["教室编号"];
