...
if (!class_exists('Database_connector')) {
    class Database_connector
    {
        ...
        public function __construct($config_file ) {
            ...
            $this->CONNECT();$this->CONNECTION_SESSION = new mysqli( $this->CONNECTION_SERVER, $this->CONNECTION_USERNAME, $this->CONNECTION_PASSWORD, $this->CONNECTION_DATABASE );
        }
        ...
        private function CONNECT(): void { // try to establish connection
            ...
            $this->CONNECTION_SESSION = new mysqli( $this->CONNECTION_SERVER, $this->CONNECTION_USERNAME, $this->CONNECTION_PASSWORD, $this->CONNECTION_DATABASE );
                               ...
                               array $order=array())
        {
            ...
            $sql = "SELECT {$sql_key} FROM {$table_name} WHERE {$sql_condition_yes} and {$sql_condition_no} and {$sql_condition_between} and {$sql_condition_in} {$sql_order};";
                               ...
                               array $conditions_between=array(), array $conditions_in=array())
        {
            ...
            $sql = "UPDATE {$table_name} SET {$sql_updateDATA} WHERE {$sql_condition_yes} and {$sql_condition_no} and {$sql_condition_between} and {$sql_condition_in};";
            $returns = $this->CONNECTION_SESSION->query($sql);
            $this->CONNECTION_SESSION->commit();
            return $returns;
        }
