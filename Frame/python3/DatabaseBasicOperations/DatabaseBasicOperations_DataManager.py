...
class DatabaseBasicOperations_DataManager:
    ...
    @staticmethod
    def downloadSelfstudyAllData(infoForm: dict, databaseConnector: DatabaseConnector | None = None) -> str:
        # =====================================
        # 如果提供已经建立的数据库连接，则直接使用
        if databaseConnector is None:
            database = DatabaseConnector()
    ...
    @staticmethod
    def downloadEmptyTimeAllData(databaseConnector: DatabaseConnector | None = None) -> str:
        # =====================================
        # 如果提供已经建立的数据库连接，则直接使用
        if databaseConnector is None:
            database = DatabaseConnector()
