								...
								<div class='col-sm-6 table-responsive'>
									<table class='table'>
										...
										<tbody id='zhouyijimingbiao'>");
											$conn_temp = new STOS_MySQL_data();
											if($a = $conn_temp->search('缺勤人员名单',false,array('日期'=>getWeekRange(time(), 1)[2][0],'提交者'=>$memberID),false)) {
								...
								<div class='col-sm-6 table-responsive'>
									<table class='table'>
										...
										<tbody id='zhouerjimingbiao'>");
											if($a = $conn_temp->search('缺勤人员名单',false,array('日期'=>getWeekRange(time(), 1)[2][1],'提交者'=>$memberID))) {
												if($t = $a->fetch_assoc()['缺勤名单']) {
													$t = json_decode($t,true);
													$count = 1;
													foreach($t as $key=>$value){
                        ...
                        <div class='row'>
                            <div class='col-sm-6 table-responsive'>
                                <table class='table'>
                                    ...
                                    <tbody id='zhouyijimingbiao'>");
                                        $conn_temp = new STOS_MySQL_data();
                                        if($a = $conn_temp->search('缺勤人员名单',false,array('日期'=>getWeekRange(time(), 1)[2][2],'提交者'=>$memberID),false)) {
                                            if($t = $a->fetch_assoc()['缺勤名单']) {
                                                $t = json_decode($t,true);
                                                $count = 1;
                                                foreach($t as $key=>$value){
                    ...
                    echo("
                            ...
                            <div class='col-sm-6 table-responsive'>
                                <table class='table'>
                                    ...
                                    <tbody id='zhouerjimingbiao'>");
                                        if($a = $conn_temp->search('缺勤人员名单',false,array('日期'=>getWeekRange(time(), 1)[2][3],'提交者'=>$memberID))) {
                                            if($t = $a->fetch_assoc()['缺勤名单']) {
                                                $t = json_decode($t,true);
                                                $count = 1;
                                                foreach($t as $key=>$value){
