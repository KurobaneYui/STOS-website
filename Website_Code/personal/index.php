					...
					<div class="col-12">
						<div class="card">
							<div class="card-block">
								...
								<h4 style="color: red" class="card-text">1.每天查早辛苦啦~ 但是要注意站姿，不能玩电脑和手机哦~</h4>
								<h4 class="card-text">2.如果没有班委可以确定缺勤学生，请直接让班级同学提供，一个班的同学，而且座位也是按学号顺序，如果还是不知道，那就直接手机拍下来教室人员情况，第二天再写名单。</h4>
								<h4 class="card-text">3.部分学院缺勤人数较多，可以不录入。如果缺勤人数大于班级人数的一半，就不用录那个班的缺勤了，直接备注一下xx班缺勤人数过多就好了（班级可以直接用学号的前10位表示）。</h4>
							</div>
						</div>
					</div>
				</div>
								...
								<div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody id="body1">
											...
											$sql = "SELECT * FROM `查早排班` WHERE `查早组员`='{$person->xuehao}' ORDER BY `周起始日期` DESC LIMIT 6;";
