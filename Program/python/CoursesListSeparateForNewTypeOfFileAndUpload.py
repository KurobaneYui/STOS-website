...
SchoolName = ['通信', '电子科学', '集成', '材料', '机械', '光电', '自动化', '资源', '计算机', '信息与软件',
...
class OldFileProcessor:
    ...
    def read_file(self, fileFullPathAndName: str, sheetName: str) -> None:
        ...
        try:
            self.fileFullPath, self.fileName = os.path.split(
                fileFullPathAndName)
            self.sheetName = sheetName
            # 把Excel数据读入DataFrame
            self.fileDataFrame = pd.DataFrame(pd.read_excel(
                fileFullPathAndName, sheet_name=self.sheetName))
            ...
            print(e)
            log_record(
                'We encounter some errors when open file and read data with Pandas\n\t'+str(e), self.fileFullPath)
    ...
    def CustomFilterFunction(self) -> None:  # 筛选校区和上课人数过少的课
        ...
        self.fileDataFrame = self.fileDataFrame[self.fileDataFrame['校区'].isin([
                                                                              '清水河校区'])]
        self.fileDataFrame = self.fileDataFrame[~self.fileDataFrame['上课人数'].isin(
            range(10))]
...
class NewFileProcessor:
    ...
    def create_from_OldFileProcessor(self, old_file_processor: OldFileProcessor) -> None:
        ...
        try:
            ...
            if self.error:
                raise Exception
            self.filebook = openpyxl.load_workbook(
                os.path.join(self.fileFullPath, self.fileName))
            ...
            print(e)
            log_record(
                'We encounter some errors when import data from original data and create new sheet in file\n\t'+str(e), self.fileFullPath)
        ...
        try:
            ...
            for row_index in range(1, self.rowNum+2):
                ...
                for cell_index in range(1, self.colNum+1):
                    ...
                    if row_index == 1:
                        self.filesheet.cell(row=row_index, column=cell_index).fill = PatternFill(
                            "solid", fgColor="AAAAAA")
                    self.filesheet.cell(row=row_index, column=cell_index).alignment = Alignment(
            ...
            for col_index in range(1, self.colNum+1):
                ...
                # 设置列宽
                self.filesheet.column_dimensions[col_str[col_index-1]
                                                 ].width = col_width[col_index-1]
            ...
            print(e)
            log_record(
                'We encounter some errors when save data into sheet and save file\n\t'+str(e), self.fileFullPath)
...
class dataNormalization:
    ...
    def normalization(self):
        ...
        for i in self.finDataFrame.index:
            ...
            cr, gd, sc = self.classroom_processor(one_row['上课教室']), self.grade_processor(
        ...
        self.finDataFrame['上课时间'] = self.finDataFrame['上课时间'].apply(
            dataNormalization.date_period_processor)
        self.finDataFrame = self.finDataFrame.explode('上课时间')
        self.finDataFrame = self.finDataFrame.reset_index(drop=True)

    def classroom_processor(self, cr):
        """对上课教室列的处理"""
        pattern = re.compile(
            r'立人楼[A-Za-z]-?[0-9]{3}|品学楼[A-Ca-c]-?[0-9]{3}-?[A-Ca-c]?')
        location_set = set(pattern.findall(cr))  # 获取符合要求的所有教室，并做去重复处理
        # 如果只有一个上课地点则保留所在行，理由：每一行对应一个老师周内的一个时间，只可能有一个上课地点，多个地点说明会根据周的不同改变，但所提供数据无法提供具体从哪一周起改变教室，故做删除处理
    ...
    @staticmethod
    def date_period_processor(dp):
        """对上课时段列的处理"""
        pattern = re.compile(
            r'(星期[一二三四五六七日])\s*(\d*-\d*)\s*(\[(\d*(-\d*)?[单双]?,?)*\])')
        pattern_result = pattern.findall(dp)

        date_periods = dict()
        for one_match in pattern_result:
            if (one_match[0]+' '+one_match[1]) not in date_periods.keys():
                date_periods[one_match[0]+' '+one_match[1]] = one_match[2]
            else:
                date_periods[one_match[0]+' '+one_match[1]] += ' '+one_match[2]
        date_periods = [key + ' ' + value for key, value in date_periods.items()]
        return date_periods
...
class UpLoader:
    ...
    def process_upload(self):
        ...
        """
        self.cur.execute(
            "select 课程编号  from `全校查课信息` order by 课程编号 DESC limit 1;")
        ...
        for row in self.oriData.index:
            ...
            if classroom_list != []:
                ...
                try:
                    ...
                    log_record("插入教室信息时出错：{}，教室如下：{}".format(
                        e, classroom_list), self.path)
            if course_list != []:
                ...
                try:
                    ...
                    log_record("插入课程信息时出错：{}，课程如下：{}".format(
                        e, classroom_list), self.path)
                else:
    ...
    def search_schoolCode_classroomCode(self, schoolName: str, classroomName: str) -> tuple:
        ...
        """
        self.cur.execute(
            "select 学院编号 from 学院信息 where 学院名称='{}';".format(schoolName))
        school_code = self.cur.fetchone()[0]

        self.cur.execute("select 教室编号 from 教室信息 where 教学楼='{}' and 区域='{}' and 牌号='{}';".format(
            self.classroom_split(classroomName)))
    ...
    def pre_process(self, one_record: Series, course_code: int) -> tuple:
        ...
        school_code, classroom_code = self.search_schoolCode_classroomCode(
            one_record['上课院系'], one_record['上课教室'])
        ...
        if classroom_code == False:
            classroom_code = self.transform_classname_to_code(
                one_record['上课教室'])
            classroom_campus = one_record['校区']
            classroom_building, classroom_area, classroom_addr = self.classroom_split(
                one_record['上课教室'])

            processedData['添加教室'].append(
                (classroom_code, classroom_campus, classroom_building, classroom_area, classroom_addr))
        processedData['添加课程'] = [(course_code+i, school_code, grade, course_name,
                                  classroom_code, date, interval, num_expect) for i, date in enumerate(dates)]
    ...
    def classroom_split(self, classroom: str) -> tuple:
        ...
        """
        classroom1 = re.compile(r'立人楼|品学楼|第二教学楼').findall(
            classroom)[0]  # FIXME:修改匹配项以适配沙河校区
        classroom_ = ''.join(re.compile(
            r'[a-zA-Z]?-?\d{3}-?[a-zA-Z]?').findall(classroom)[0].split('-'))  # FIXME:修改匹配项以适配沙河校区
    ...
    def date_transform(self, week: str, weekday: str) -> list:
        ...
        for week_group in weeks:
            ...
            for i in range(start_week, end_week+1):
                if odd_even == 1 and i % 2 == 0:
                    continue
                if odd_even == 2 and i % 2 == 1:
                    continue
        ...
        if input_len >= 4:
            with open(sys.argv[1], 'r') as f:
                with connector.connect(**json.load(f)) as session:
                    ...
                    date_useful_cols = [sys.argv[i]
                                        for i in range(5, input_len+1)]
                    if mode == 0:
                        OLD = OldFileProcessor()  # 初始化预处理
                        OLD.read_file(path, sheetName)  # 设置文件和表单
                        returns = json.dumps(
                            dict(enumerate(OLD.show_colName())))  # 显示列名
                        print(json.dumps({'ReturnCode': '200', 'ReturnString': '成功',
                              'ShowMessage': '', 'Data': returns}, ensure_ascii=False))
                    elif mode == 1:
