...
# 写入Excel文件数据
def writedata(path: str, session: connector.connection.MySQLConnection,
    ...
    # 读数据库
    cur = session.cursor()
    cur.execute("select * from 成员工资信息")
    results = cur.fetchall()
    column_names = cur.column_names  # 表头信息
    # Wage表单写入表头
    ...
    # Info表单写入表头
    ws_sheet2.cell(row=1, column=1, value='{}年{}月勤工助学补助'.format(date.year, date.month))
    ws_sheet2.cell(row=2, column=1, value="指导老师姓名：{}        指导老师电话：{}         指导老师邮箱：{}".format(teacherName, teacherPhone, teacherEmail))
    ws_sheet2.cell(row=3, column=1, value='骨干姓名：{}         骨干电话：{}          骨干邮箱：{}         岗位总数：{}         办公地点：{}								'.format(teamLeaderName, teamLeaderPhone, teamLeaderEmail,len(results), workPlace))
    ws_sheet2.cell(row=4, column=1, value='一档金额：{}元/月（人）   二档金额：{}元/月（人）            三档金额: {}元/月（人）            建档立卡专设岗位 {}（人）'.format(firstWage, secondWage, thirdWage, numForSubsidy))
    # 数据写入Info表单
    ws_sheet2.cell(row=5,column=1,value="序号")
    for conlum_index in range(2,len(column_names)+2):
        ws_sheet2.cell(row=5,column=conlum_index,value=column_names[conlum_index-2]) # 写表头
    for row_index in range(1,len(results)+1):
        ...
        for col_index in range(2,len(column_names)+2):
            ws_sheet2.cell(row=row_index+5,column=col_index,value=results[row_index-1][col_index-2]) # 写入表的内容
    cur.close()
    ws_sheet2.cell(row=5+len(results)+1,column=column_names.index('基本工资')+1,value='总金额') # 写入‘总金额’
...
def SetStyle(path: str):
    ...
    ws_sheet2.cell(row=1, column=1).font= Font(u'楷体', size=20, bold=True, color='000000')
    ws_sheet2.cell(row=1, column=1).alignment = Alignment(horizontal='center', vertical='center')


    # 2到4行的格式
    for R in range(2,5):
        ws_sheet2.row_dimensions[R].height = 24
        ws_sheet2.cell(row= R, column=1).alignment = Alignment(horizontal='center', vertical='center')
        ws_sheet2.cell(row=R, column=1).font = Font(u'楷体', size=12, bold=True, color='000000')
    ...
    for i in range(1,ncols+1): # 第五行没合并就单独拿出来了
        ws_sheet2.cell(row=5, column=i).font = Font(u'楷体', size=12, bold=True, color='000000')

    trows = 0
    for r in range(5, nrows + 1): # 数据格式
         for l in range(1,ncols + 1):
             ws_sheet2.row_dimensions[r].height = 21
             ws_sheet2.cell(row=r, column=l).alignment = Alignment(horizontal='center', vertical='center')
             ws_sheet2.cell(row=r, column=l).font = Font(u'楷体', size=12, color='000000')
             if ws_sheet2.cell(row=r, column=1).value == None: # 由于最下面总金额和下面的注意事项不放在边框内，所以真实行数不算他们
                 trows = r - 2
        ...
        for l in range(1, ncols + 1):
            ...
            if r == trows and l == ncols:
                ws_sheet2.cell(row=r, column=l).border = Border(left=Side(border_style='thin', color='000000'),
                                                                ...
                                                                bottom=Side(border_style='medium', color='000000'))
    # 注意事项的格式设置
    ws_sheet2.merge_cells(start_row=trows+3, start_column=1, end_row=trows+13, end_column=ncols)
    ...
    ws_sheet1.cell(row=1, column=1).font = Font(u'楷体', size=20, bold=True, color='000000')
    ws_sheet1.cell(row=1, column=1).alignment = Alignment(horizontal='center', vertical='center')

    # 2到4行的格式
    for R in range(2, 5):
        ws_sheet1.row_dimensions[R].height = 24
        ws_sheet1.cell(row=R, column=1).alignment = Alignment(horizontal='center', vertical='center')
        ws_sheet1.cell(row=R, column=1).font = Font(u'楷体', size=12, bold=True, color='000000')
    ...
    ws_sheet1.cell(row=nrows + 1, column=4 + 1).font = Font(u'楷体', size=12, color='000000')
    ws_sheet1.cell(row=trows + 5, column=1,
                   value='1、各单位需完整填写表内各项，以便账号等出现问题可以及时取得联系。\n2、若该同学已离开该单位只需将该同学删除即可。新加入的同学请添加于最后，并用红色字体标出。补发的同学添加于新加入的同学之后，在发放月份栏注明“补发x月”。\n3、若该同学补助金额不为标准补助金额即对应的岗位设档金额时，请在备注说明原因。\n4、表的最后需计算总计金额。本表不能代替交至学工部的Word文档表。\n5、Excel表格及邮件主题名称为：部门+2019年 x 月）。\n6、请各单位注意核对同学（尤其是少数民族学生）的账号和姓名等信息，非建行新鸿支行卡不可用。\n7、若有建档立卡岗，请备注。\n8、第四行的设档信息是指申请并审核通过的设档信息，所以这一学期都是不会变化的。请勿错填成当月的成员信息。\n')

    for i in range(1, ncols + 1):  # 第五行没合并就单独拿出来了
        ws_sheet1.cell(row=5, column=i).font = Font(u'楷体', size=12, bold=True, color='000000')
        ws_sheet1.cell(row=5, column=i).alignment = Alignment(horizontal='center', vertical='center')
    for r in range(5, nrows + 1): # 数据格式
         for l in range(1,ncols + 1):
             ws_sheet1.row_dimensions[r].height = 21
             ws_sheet1.cell(row=r, column=l).alignment = Alignment(horizontal='center', vertical='center')
             ws_sheet1.cell(row=r, column=l).font = Font(u'楷体', size=12, color='000000')
    ...
    # 注意事项的格式设置
    ws_sheet1.merge_cells(start_row=trows + 5, start_column=1, end_row=trows + 15, end_column=ncols)
    for i in range(trows + 3, trows + 11):
        for j in range(1, ncols + 1):
            ws_sheet1.cell(row=i, column=j).alignment = Alignment(wrapText=True)
            ws_sheet1.cell(row=i, column=j).font = Font(u'楷体', size=12,  color='000000')
        ...
        if input_len==14:
            with open(sys.argv[1], 'r') as f:
                with connector.connect(**json.load(f)) as session:
                    path = os.path.join(sys.argv[2],str(time.time()))
                    ...
                    params = {
                        ...
                        'teamLeaderName': sys.argv[7],
                        'teamLeaderPhone': sys.argv[8],
                        'teamLeaderEmail': sys.argv[9],
                        'workPlace': sys.argv[10],
                        'firstWage': float(sys.argv[11]),
                        'secondWage': float(sys.argv[12]),
                        'thirdWage': float(sys.argv[13]),
                        'numForSubsidy': int(sys.argv[14])
                    }
                    writedata(**params)
                    SetStyle(path)
        else:
