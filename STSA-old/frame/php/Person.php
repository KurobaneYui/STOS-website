    ...
    class Person
    {
        ...
        public function __construct(string $studentID) {
            ...
            $this->informationValid['all']
                = $this->informationValid['basic']
                = $this->informationValid['work']
                = $this->informationValid['password']
                = $this->informationValid['authorization']
        ...
        public function fetch_basic_info(): bool {
            ...
            if($result = $result->fetch_assoc()) {
                ...
                $this->dormitory_lou = $result['寝室_楼'];
        ...
        public function fetch_work_info(): bool {
            ...
            if ($result = $result->fetch_assoc()) {
                ...
                $results = $this->STOS_DATABASE_INFORMATION->search('部门信息', array('部门','组长'), array('组长' =>$this->studentID));
        ...
        // private function: check if personal information is valid
        // ！！！！ “学院、民族、籍贯、备注” 只去除特殊标签和字符； “权限” 未检测！！！
        /**
         * @return void
         */
        private function check_data(): void { // check all information
            $group_list = $this->STOS_DATABASE_INFORMATION->search('部门信息',array('部门'));
            ...
            foreach($basic_filter_result as $key=>$value) {
                ...
                if (!$value) {
                    $this->informationValid['basic'] = false;
                    $this->invalid_infos['basic'][$key] = $value;
                }
            }
            foreach($work_filter_result as $key=>$value) {
                ...
                if (!$value) {
                    $this->informationValid['work'] = false;
                    $this->invalid_infos['work'][$key] = $value;
                }
            }
            foreach($password_filter_result as $key=>$value) {
                ...
                if (!$value) {
                    $this->informationValid['password'] = false;
                    $this->invalid_infos['password'][$key] = $value;
                }
            }
            foreach($authorization_filter_result as $key=>$value) {
                ...
                if (!$value) {
                    $this->informationValid['authorization'] = false;
                    $this->invalid_infos['authorization'][$key] = $value;
                }
            }

            $this->informationValid['all'] = (
                $this->informationValid['basic']
                and $this->informationValid['work']
                and $this->informationValid['password']
                and $this->informationValid['authorization']
            );
        }
