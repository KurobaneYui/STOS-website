...
def log_record(log: str, save_dir: str) -> None:  # 用于记录错误信息
    ...
    with open(
        os.path.join(save_dir, "log.log"), "a+"
    ) as f:  # 以“继续添加”模式打开文件，如果不存在则自动创建文件
        f.write(log + "\n\n")  # 两次记录之间有一个空行，方便查看


class OldFileProcessor:
    """全校课表Excel数据读入\n
    提供无用列删除，无用行删除的功能，提供简单的自定义筛除功能，如筛选校区和实际人数大于10人的课等"""
    ...
    def read_file(self, fileFullPathAndName: str, sheetName: str) -> None:
        ...
        try:
            ...
            # 把Excel数据读入DataFrame
            self.fileDataFrame = pd.DataFrame(
                pd.read_excel(fileFullPathAndName, sheet_name=self.sheetName)
            )
    ...
    def CustomFilterFunction(self) -> None:  # 筛选校区和实际人数过少的课
        """自定义函数：\n
        筛选校区，保留清水河校区课程\n
        去除实际人数少于10人的教室"""
        # 筛选出“校区”列内容为“清水河校区”的行
        self.fileDataFrame = self.fileDataFrame[
            self.fileDataFrame["校区"].isin(["清水河校区"])
        ]
        self.fileDataFrame = self.fileDataFrame[
            ~self.fileDataFrame["实际人数"].isin(range(10))
        ]
...
class NewFileProcessor:
    ...
    def create_from_OldFileProcessor(
        self, old_file_processor: OldFileProcessor
    ) -> None:
        """从预处理程序读取数据"""
        try:
    ...
    def save_data_and_file(self) -> None:
        ...
        try:
            ...
            for col_index in range(1, self.colNum + 1):
                ...
                # 设置列宽
                self.filesheet.column_dimensions[
                    col_str[col_index - 1]
...
class dataNormalization:
    ...
    def normalization(self):
        ...
        for i in self.finDataFrame.index:
            ...
            cr, gd, sc = (
                self.classroom_processor(one_row["上课地点"]),
                self.grade_processor(one_row["年级"]),
                self.school_processor(one_row["上课院系"]),
            )
            if False in [cr, gd, sc]:
                self.finDataFrame = self.finDataFrame.drop(index=i)
            else:
                self.finDataFrame.at[i, "上课地点"] = cr
    ...
    def school_processor(self, sc):
        ...
        count = 0
        name = ""
        for n in college_name:  # 只有一个学院则保留，多学院则删除行
            if sc.find(n) != -1:
                count += 1
                name = n
        if count == 1:
...
class UpLoader:
    ...
    def __init__(self, oriData: NewFileProcessor, session: Session, date: str) -> None:
        """
    ...
    def process_upload(self):
        ...
        for row in self.oriData.index:
            ...
            if course_list != []:
                sql = (
                    "INSERT INTO `全校查课信息`(`课程编号`, `学院编号`, `年级`, `课程名称`, `教室编号`,`日期`,`时段`, `应到人数`)\
    ...
    def search_schoolCode_classroomCode(
        self, schoolName: str, classroomName: str
    ) -> tuple:
        """
        搜索学院编号和教室编号
        """
        ...
        self.cur.execute(
            "select 教室编号 from 教室信息 where 教学楼='{}' and 区域='{}' and 牌号='{}';".format(
    ...
    def pre_process(self, one_record: Series, course_code: int) -> tuple:
        ...
        school_code, classroom_code = self.search_schoolCode_classroomCode(
            one_record["上课院系"], one_record["上课地点"]
        )
        ...
        num_expect = int(one_record["实际人数"])

        if classroom_code == False:
            classroom_code = self.transform_classname_to_code(one_record["上课地点"])
    ...
    def classroom_split(self, classroom: str) -> tuple:
        ...
        """
        classroom1 = re.compile(r"立人楼|品学楼|第二教学楼").findall(classroom)[
            0
        ]  # FIXME:修改匹配项以适配沙河校区
        classroom_ = "".join(
            re.compile(r"[a-zA-Z]?-?\d{3}-?[a-zA-Z]?").findall(classroom)[0].split("-")
        )  # FIXME:修改匹配项以适配沙河校区
...
if __name__ == "__main__":
    ...
    try:
        ...
        if input_len >= 3:
            ...
            date_useful_cols = [sys.argv[i] for i in range(4, input_len + 1)]
            if mode == 0:
                OLD = OldFileProcessor()  # 初始化预处理
                OLD.read_file(path, sheetName)  # 设置文件和表单
                returns = dict(enumerate(OLD.show_colName()))
            elif mode == 1:
                ...
                raise Exception(
                    "模式选择错误，选择的模式是{}，而该模式不存在".format(mode)
                )
        else:
            raise Exception(
