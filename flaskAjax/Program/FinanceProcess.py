...
from flaskAjax.BaseComponents.DatabaseConnector import SQL_WageView, SessionLocal
from openpyxl.utils import get_column_letter


def wageView2dict(wage_view) -> dict:
    return {
        "name": wage_view.name,
        ...
        "application_student_id": wage_view.application_student_id,
        "application_bankcard": wage_view.application_bankcard,
        "subsidy_dossier": ["否", "是"][wage_view.subsidy_dossier],
        "wageinfo_remark": wage_view.wageinfo_remark,
    }
...
def writedata(
    ...
    results_db = database.query(SQL_WageView).all()
    ...
    # Wage表单写入表头
    ws_sheet1.cell(
        row=1, column=1, value="{}年{}月勤工助学补助".format(date.year, date.month)
    )
    ws_sheet1.cell(
        row=2,
        column=1,
        value="指导老师姓名：{}        指导老师电话：{}         指导老师邮箱：{}".format(
    ...
    ws_sheet1.cell(
        row=3,
        column=1,
        value="骨干姓名：{}         骨干电话：{}          骨干邮箱：{}         岗位总数：{}         办公地点：{}".format(
    ...
    ws_sheet1.cell(
        row=4,
        column=1,
        value="一档金额：{}元/月（人）   二档金额：{}元/月（人）            三档金额: {}元/月（人）            建档立卡专设岗位 {}（人）".format(
    ...
    for conlum_index in range(2, len(column_names) + 2):
        ws_sheet2.cell(
            row=5, column=conlum_index, value=column_names[conlum_index - 2]
        )  # 写表头
    for row_index in range(1, len(results) + 1):
        ...
        for col_index in range(2, len(column_names) + 2):
            ws_sheet2.cell(
                row=row_index + 5,
                column=col_index,
                value=results[row_index - 1][column_database_name[col_index - 2]],
            )  # 写入表的内容
    ws_sheet2.cell(
        row=5 + len(results) + 1,
        column=column_names.index("基本工资") + 1,
        value="总金额",
    )  # 写入‘总金额’
    ws_sheet2.cell(
        row=5 + len(results) + 1 + 2,
        column=1,
...
def SetStyle(path: str):
    ...
    ws_sheet2.cell(row=1, column=1).font = Font(
        "楷体", size=20, bold=True, color="000000"
    )
    ...
    for R in range(2, 5):
        ...
        ws_sheet2.cell(row=R, column=1).font = Font(
            "楷体", size=12, bold=True, color="000000"
        )
    ...
    for r in range(5, nrows + 1):  # 数据格式
        for l in range(1, ncols + 1):
            ...
            ws_sheet2.cell(row=r, column=l).font = Font("楷体", size=12, color="000000")
    ...
    for r in range(1, trows + 1):
        for l in range(1, ncols + 1):
            ws_sheet2.cell(row=r, column=l).border = Border(
    ...
    # 注意事项的格式设置
    ws_sheet2.merge_cells(
        start_row=trows + 3, start_column=1, end_row=trows + 13, end_column=ncols
    )
    for i in range(trows + 3, trows + 11):
        for j in range(1, ncols + 1):
            ws_sheet2.cell(row=i, column=j).alignment = Alignment(wrapText=True)
            ws_sheet2.cell(row=i, column=j).font = Font("楷体", size=12, color="000000")
    ...
    ws_sheet1.cell(row=nrows + 1, column=4 + 1).font = Font(
        "楷体", size=12, color="000000"
    )
    ws_sheet1.cell(
        row=trows + 5,
        column=1,
    ...
    for r in range(5, nrows + 1):  # 数据格式
        for l in range(1, ncols + 1):
            ...
            ws_sheet1.cell(row=r, column=l).font = Font("楷体", size=12, color="000000")
    ...
    for r in range(1, nrows + 1):
        for l in range(1, ncols + 1):
            ws_sheet1.cell(row=r, column=l).border = Border(
                left=Side(border_style="thin", color="000000"),
                right=Side(border_style="thin", color="000000"),
                top=Side(border_style="thin", color="000000"),
                bottom=Side(border_style="thin", color="000000"),
    ...
    # 注意事项的格式设置
    ws_sheet1.merge_cells(
        start_row=trows + 5, start_column=1, end_row=trows + 15, end_column=ncols
    )
    for i in range(trows + 3, trows + 11):
        for j in range(1, ncols + 1):
            ws_sheet1.cell(row=i, column=j).alignment = Alignment(wrapText=True)
            ws_sheet1.cell(row=i, column=j).font = Font("楷体", size=12, color="000000")
