...
def RegisterCheck(formDict: dict, db_session: Session) -> dict[str, Any]:
    ...
    DBAffectRows = db_session.query(College)
    if DBAffectRows != 1:
    ...
    def submitSelfstudyRecordParamsCheck(infoForm: dict) -> None:
        ...
        for one_student in infoForm["absentList"]:
            ...
            if any(char.isdigit() for char in one_student["student_name"]) or not any(
                char.isalpha() and char.isalnum() for char in one_student["student_name"]
            ):
                raise IllegalValueError(
            ...
            if not (
                one_student["student_id"].isalnum() and
                one_student["student_id"].isascii()
            ):
                raise IllegalValueError(
