...
const onSubmit = async () => {
  ...
  try {
    ...
    if (code === 400) {
      ...
      if (code === 301) { window.console.log('登录函数移至新位置'); }
...
// 选择岗位后登录
const loginAsSpecifiedWork = async (department_id = 0, job = 0) => {
  try {
...
<template>
  <!-- 岗位选择弹窗 -->
  <div class="modal fade" ref="selectWorkModal" id="select-login-work" tabindex="-1"
          ...
          <div class="row g-2" id="work-list-container">
            ...
            <template v-else>
              <div
                ...
                :key="work.department_id + '-' + work.job"
                class="col-auto"
              >
                <button
